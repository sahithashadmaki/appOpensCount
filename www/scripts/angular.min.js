 ...
 * </example>
 */
var ngModelDirective = function($compile, $rootScope) {
  return {
    ...
    link: function(scope, element, attr, ctrls) {
      ...
      if(attr.dynamicname) {
    	  scope.formProps = $rootScope.$$childHead.form_props? $rootScope.$$childHead.form_props : null;
    	  var validationExtra = '';
    	  if(scope.form_props) {
    		  validationExtra = '&& formProps.showValidation';
    	  }
    	  scope.show_form_element_errors = false;

    	  modelCtrl.$name = scope.$eval(attr.dynamicname);

    	  var node_name = modelCtrl.$name,
    	  	  $scope = scope,
    	  	  node = element[0],
    	  	  div = '<div class="bubbletooltip arrow-up" data-ng-show="!'+formCtrl.$name+'.' + node_name
    	  	  		+ '.$valid '+validationExtra+' && show_form_element_errors">'
					+ '<div data-ng-show="'+formCtrl.$name+'.'+node_name+'.$error.nsRequired">This field is required.</div>'
					+ '<div data-ng-show="'+formCtrl.$name+'.'+node_name+'.$error.nsNumber">This field accepts only numeric values.</div>'
					+ '<div data-ng-show="'+formCtrl.$name+'.'+node_name+'.$error.required">This field is required.</div>'
					+ '<div data-ng-show="'+formCtrl.$name+'.'+node_name+'.$error.nsMinLength">This field needs to be at least '
					+ '{{' + $(node).attr('data-ns-min-length') + '}} characters in length.</div>'
					+ '<div data-ng-show="'+formCtrl.$name+'.'+node_name+'.$error.nsMaxLength">This field needs to be no more than '
					+ '{{' + $(node).attr('data-ns-max-length') + '}} characters in length.</div>'
					+ '<div data-ng-show="'+formCtrl.$name+'.'+node_name+'.$error.dateGreaterThan">'
					+ $(node).attr('data-date-greater-than-msg') + '</div>'
					+ '<div data-ng-show="'+formCtrl.$name+'.'+node_name+'.$error.email">'
					+ 'Please enter a valid e-mail address</div>'
					+ '<div data-ng-show="'+formCtrl.$name+'.'+node_name+'.$error.invalidFormat">'
					+ 'The value you have entered is invalid.</div>'
              	    + '<div data-ng-show="'+formCtrl.$name+'.'+node_name+'.$error.nsMin">This value needs to be at least '
              		+ '{{' + $(node).attr('data-ns-min') + '}}</div>'
             		+ '<div data-ng-show="'+formCtrl.$name+'.'+node_name+'.$error.nsMax">This value needs to be no more than '
              		+ '{{' + $(node).attr('data-ns-max') + '}}</div>'
             		+ '<div data-ng-show="'+formCtrl.$name+'.'+node_name+'.$error.checkAddedQues">You must have more questions</div>'
					+ '</div>';

  			var element_highlight = node.type == 'hidden'? $(node).parent() : node;
  			$(element_highlight).focusout(function() {
  				$scope.$apply(function () {
  					$scope.show_form_element_errors = false;
  				});
  			});
  			$(element_highlight).hover(function() {
  				$scope.$apply(function () {
  					$scope.show_form_element_errors = true;
  				});
  			}, function() {
  				$scope.$apply(function () {
  					if(!$(node).is(':focus')) {
 ...
 */
var ngShowDirective = function($animate) {
  return function(scope, element, attr) {
